...
def report(
    ...
    title: str = typer.Option("EDA-отчет", help="Заголовок отчёта в Markdown."),
    min_missing_share: int = typer.Option(0.5, help="Порог доли пропусков, выше которого колонка считается проблемной."),
) -> None:
    """
    ...
    with md_path.open("w", encoding="utf-8") as f:
        ...
        f.write(f"- Макс. гистограмм числовых колонок: `{max_hist_columns}`\n")
        f.write(f"- Количество top-значений для категорий: `{top_k_categories}`\n")
        ...
        if not top_cats:
            ...
            f.write(f"Топ-{top_k_categories} значений для каждой категориальной колонки:\n\n")
        ...
        if max_hist_columns ==0:
            f.write("Гистограммы сгенерированы для всех числовых колонок.\n")
        else:
            f.write(f"Гистограммы сгенерированы для первых {max_hist_columns} числовых колонок.\n")
