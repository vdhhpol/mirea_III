...
@app.post(
    ...
    response_description = "Словарь со всеми флагами качества из HW03",
)
async def quality_flags_from_csv(file: UploadFile = File(...)) -> dict:
    ...
    включая новые флаги из HW03 (has_constant_columns, has_high_cardinality_categoricals, 
    ...
    if df.empty:
        raise HTTPException(
            status_code=400,
            detail = "CSV-файл не содержит данных (пустой)."
        )
    ...
    for flag_name in bool_flags:
        if flag_name in flags_all:
            flags_dict[flag_name] = bool(flags_all[flag_name])

    if "quality_score" in flags_all:
        flags_dict["quality_score"] = float(flags_all["quality_score"])
    if "max_missing_share" in flags_all:
        flags_dict["max_missing_share"] = float(flags_all["max_missing_share"])

    try:
        flags_dict["dataset_shape"] = {
            "n_rows": int(getattr(summary, "n_rows")),
            "n_cols": int(getattr(summary, "n_cols"))
        }
    except AttributeError:
    ...
    print(f"[quality-flags-from-csv] filename={file.filename!r} shape={df.shape} flags={flags_dict}")
