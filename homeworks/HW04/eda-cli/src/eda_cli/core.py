...
def compute_quality_flags(df: pd.DataFrame, summary: DatasetSummary, missing_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    #has_high_cardinality_categoricals — флаг, сигнализирующий, что есть категориальные признаки с очень большим числом уникальных значений.
    ...
    for col in summary.columns:
        if col.is_numeric and (col.name in df.columns):
            ...
            zero_share = zero_count / summary.n_rows if summary.n_rows > 0 else 0
